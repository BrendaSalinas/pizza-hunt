...
function getPizza() {
  ...
  .then(response => {
    //if there's no pizza with specific id throw an error
    if(!response.ok) {
  ...
  .then(printPizza)
  //Add a catch error so any error will send the user back to the homepage
  .catch(err => {
    console.log(err);
...
function handleNewCommentSubmit(event) {
  ...
  //Adding a fetch POST to allow the user to post a comment on the discussion section
